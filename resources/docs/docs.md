...
The `home-routes` handler in the `livescore-scrapper.routes.home` namespace
...
The `home-page` function will in turn call the `livescore-scrapper.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `livescore-scrapper.handler` namespace:
...
Request middleware functions are located under the `livescore-scrapper.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `livescore-scrapper.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `livescore-scrapper.db.core` in some other namespace.
